...
const requireRestSample: IExerciseType = {
  ...
  describe('Service methods', () => {
    ...
    describe('addExerciseTypeToCollectionIfMissing', () => {
      it('should add a ExerciseType to an empty array', () => {
        const exerciseType: IExerciseType = sampleWithRequiredData;
      ...
      it('should not add a ExerciseType to an array that contains it', () => {
        const exerciseType: IExerciseType = sampleWithRequiredData;
        const exerciseTypeCollection: IExerciseType[] = [
          {
            ...exerciseType,
          },
      ...
      it("should add a ExerciseType to an array that doesn't contain it", () => {
        const exerciseType: IExerciseType = sampleWithRequiredData;
        const exerciseTypeCollection: IExerciseType[] = [sampleWithPartialData];
        expectedResult = service.addExerciseTypeToCollectionIfMissing(exerciseTypeCollection, exerciseType);
        expect(expectedResult).toHaveLength(2);
        expect(expectedResult).toContain(exerciseType);
      });
      ...
      it('should add only unique ExerciseType to an array', () => {
        const exerciseTypeArray: IExerciseType[] = [sampleWithRequiredData, sampleWithPartialData, sampleWithFullData];
        const exerciseTypeCollection: IExerciseType[] = [sampleWithRequiredData];
      ...
      it('should accept varargs', () => {
        const exerciseType: IExerciseType = sampleWithRequiredData;
        const exerciseType2: IExerciseType = sampleWithPartialData;
